...
def check_for_updates(args):
    ...
    # Get the most recent tag across all branches
    git_latest_tag_cmd = "git describe --tags $(git rev-list --tags --max-count=1)"
    latest_tag = subprocess.check_output(git_latest_tag_cmd, shell=True, cwd=args.root)
...
def update(args):
    ...
    get_release_key = ['gpg', '--recv-key', '22245C81E3BAEB4138B36061310F561200F4AD77']
    ...
    sig_result = subprocess.check_output(git_verify_tag_cmd,
                            stderr=subprocess.STDOUT, cwd=args.root)
...
def parse_argv(argv):
    ...
    parse_update = subparsers.add_parser('check_for_updates', help=update.__doc__)
    parse_update.set_defaults(func=check_for_updates)
