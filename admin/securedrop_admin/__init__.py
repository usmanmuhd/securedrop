...
def check_for_updates(args):
    ...
    git_latest_commit_cmd = ["git", "rev-list", "--tags", "--max-count=1"]
    commit_hash = subprocess.check_output(git_latest_commit_cmd, cwd=args.root)

    # Get latest tag
    git_latest_tag_cmd = ["git", "describe", "--tags",
                          commit_hash.rstrip("\n")]
    latest_tag = subprocess.check_output(git_latest_tag_cmd, cwd=args.root)
